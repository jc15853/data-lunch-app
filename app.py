...
# -----------------------------------------------------------------------------
# TAB 3: 조건문 필터링 (행사일/비고 기준 필터링으로 변경)
# -----------------------------------------------------------------------------
with tab3:
    st.subheader("3️⃣ 조건문(Logic)을 활용한 특정 조건 급식일 추출")
    
    event_filter = st.checkbox("잔반 없는 날 행사일만 보기", value=False)
    
    if event_filter:
        filtered_menu = df[df['비고'].str.contains("잔반 없는 날")]
        st.write("📋 **'잔반 없는 날' 지정 급식일 목록**")
    else:
        filtered_menu = df
